...
public class SimulationService {
    ...
    public SimulationResponse runSimulation(SimulationRequest request) {
        ...
        double speed = request.throwCondition().speed();
        ...
        if (!trajectory.isEmpty() && x >= targetX) {
            int lastIndex = trajectory.size() - 1;
            TrajectoryPoint p = trajectory.get(lastIndex);
            // xを正確にターゲット距離にし、yは直前の傾きから直線補間
            trajectory.set(lastIndex, new TrajectoryPoint(p.time(), targetX, p.y(), p.z()));
        }
